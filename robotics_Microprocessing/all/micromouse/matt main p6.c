...
volatile int speed_left = 30;
volatile int speed_right = 30;
volatile int top_speed = 30;
	...
	for(;;) {
		...
		switch (state){	
			case 0 :	PTH = 0x40;  // 0
						...
						break;
			case 1 :	PTH = 0x79;  // 1 
						HALF_STEP_FORWARD(910);
						NEXT_STATE();
						break;
			case 2 :	PTH = 0x24;  // 2
						STATIONARY_TURN(208, direction);	// 90-TURN - ramp up
						NEXT_STATE();
						break;
            case 3 :	HALF_STEP_FORWARD(780);
						NEXT_STATE();
						break;
			case 4 :	PTH = 0x19;  // 4
			...
			case 7 :	HALF_STEP_FORWARD(820);
						NEXT_STATE();
						break;
			case 8 :  PTH = 0x00; // 8	
						STATIONARY_TURN(416, direction);	// U-TURN
...
void HALF_STEP_FORWARD(int steps){
	...
	while (count_both <= steps){
		if ((state == 1 || state == 3 || state == 5 || state == 7) && y == 55 && (steps < 150)){	// intermediate value theorem - Matt (haven't tested)
			count_both = steps + 1;
			//state = 8; // u turn
		} else {
			//PD CONTROL
