	...
	for(;;) {
      ...
      switch (state){	
			case 0 :  RAMP_BOTH_UP();
			              NEXT_STATE();
			              break;
			case 1 :  HALF_STEP_FORWARD(steps);
                          NEXT_STATE();
                           break;
			case 2 :  STATIONARY_TURN(200, direction);	// 90-TURN - ramp up
				          NEXT_STATE();
				          break;
            case 3 : HALF_STEP_FORWARD(steps);
                          NEXT_STATE();
                           break;
			case 4 :  PIVOT_TURN(100, direction);
			              NEXT_STATE();
			              break;	//PIVOT_TURN (ramp_one_down + ramp_one_up)
            case 5 : HALF_STEP_FORWARD(steps);
                          NEXT_STATE();
                          break;
			case 6 : ROLLING_TURN(100, direction);
			              break;	//ROLLING_TURN (set both speeds 1:3 ratio)
            case 7 : HALF_STEP_FORWARD(steps);
                          NEXT_STATE();
                           break;
  ...
  while (count_one <= steps){
		...
		if (direction){	// if direction = 1 (turning right)
		  if (current_speed <= bottom_speed){
			...
			if (interrupcount_left >= top_speed){	// constant speed
		  ...
		  if (current_speed <= bottom_speed){
			if (interrupcount_left >= current_speed){	// slow down left wheel (turning left)
