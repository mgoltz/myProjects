...
interrupt VectorNumber_Vtimmdcu void mdcuInterrupt () 
{
  ...
  //	assigning kd a larger value seems to cause the bot to overreact to a change in distance
	static const float kd = 20;	
	static const float kp = 0.6; // play with this number a bit 0.6 was good also
  ...
  if (state == 0){
      ...
      if (y > 55)
        ...
        //PD Control  
       /* 
        if (Output_left[OutputIndex] < 55) {  // left sensor no wall right sensor operation
          
          if (Output_right[OutputIndex] > 75){
            ...
            errorDot_r = Output_right[OutputIndex] - Output_right[(OutputIndex - 1 == -1? 1 : 0)];	//represents change in position over last two readings
	          speed_right = speed - (int)(kp * error_r - kd * errorDot_r);  //decrease speed of left wheel
          }
	        //Output_left[0] = 0;
          //Output_left[1] = 0;
	  	
	  	    else if (Output_right[OutputIndex] < 75 ){
        ...
        */
        ...
        /*if (Output_right[OutputIndex] < 55){
         
         	error_r = 80 - Output_right[OutputIndex];
	        errorDot_r = Output_right[OutputIndex] - Output_right[(OutputIndex - 1 == -1? 1 : 0)];
	  
	        speed_right = 11 + (int)(kp * error_r - kd * errorDot_r);
	
	         if (speed_right > 14)
              speed_right = 14;
	         else if (speed_right < 9)
		          speed_right = 9; 
          
        } else*/ //if (Output_right[OutputIndex] > 75){
        
          /*if (Output_left[OutputIndex] > 75){
          ...
          else if (Output_left[OutputIndex] < 75 ){
	  	     speed_right = speed_left - 1; 
	  	    } */
	  	...
	  	//if (Output_left[OutputIndex] > 68){ // left wall present
	  	if ((Output_left[OutputIndex] > Output_right[OutputIndex]) && (diff > 10)){ // left wall present
	  	    
	  	    PTH = 0x79; // right one
	  	      
	  	    //if (Output_left[OutputIndex] < 70){ // same speed to wall
         ...
         if (Output_left[OutputIndex] < 80) { // slow down left wheel away from wall
            speed_right = 19;
            	...
            	error_r = 71 - Output_left[OutputIndex];
      ...
      //else if (Output_right[OutputIndex] > 65) {
      else if ((Output_right[OutputIndex] > Output_left[OutputIndex]) && (diff > 10)) {
        
          PTH = 0x4F; // left one
          
          //if (Output_right[OutputIndex] < 70){
          ...
          if (Output_right[OutputIndex] < 80) {
            ...
            error_l = 71 - Output_right[OutputIndex];
