...
int duty = 0;
int prev8 = 0;  // add up 8 conversions
int count8 = 950; // count 8 conversions
int prev50 = 0; // add up previous 50 conversions
int count50 = 1; // count 50 conversions
...
interrupt VectorNumber_Vrti void rtiInterrupt () {
  ...
  * * * * * * * * * * * * * * */
  ...
  MSDelay(1);
  DATWRT4(buffer[2]);   // insert char of next digit
  MSDelay(1);

  }
  // write to second line
  COMWRT4(0xC5);  // second line
  DATWRT4('5');   // insert char of first digit
  MSDelay(1); 
  DATWRT4('0');   // insert char of next digit
  MSDelay(1);
  DATWRT4('0');   // insert char of next digit
  MSDelay(1);
